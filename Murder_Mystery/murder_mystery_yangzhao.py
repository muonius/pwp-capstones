...
# Add escape \ in front of special characters to return the entire note as one string
...
def prepare_text(raw_text):
    prepared_text=raw_text.lower().replace("!"," ").replace("?"," ").replace("."," ").split()
    return prepared_text

def ngram_creator(prepared_text):
    ngram_list=[]
...
class TextSample:
    def __init__(self, raw_text, author):
        ...
        self.ngram_frequency=build_frequency_table(ngram_list)  
        
    def __repr__(self):
        return "The author's name is " + (self.author) +". The average sentence length is " + str(self.average_sentence_length) +"."
...
#create frequency comparison score
...
def find_text_similarity(x,y):
    ...
    word_count_similarity=frequency_comparison()
    ngram_similarity=frequency_comparison()
...
Lily sentence length difference is 0.08216829333
Myrtle sentence length difference is 0.2657876475
Greg sentence length difference is 0.1209904333

Lily word count similairty score is 0.05
Mythre word count similarity score is 0.01
Gregg word count similarity score is 0.02
